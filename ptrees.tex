...
%          };
%      \end{tikzpicture}
%	  \caption{A decision tree over vacations}
%	\end{figure}
%}
%
%\frameT{Example: Preferences over Vacations}{
...
%\frameT{Preference Trees: Example}{
%	\begin{center}
%		$\vph_1=x_1 \lor x_3$, $\vph_2 =x_1 \land \neg x_2$, \\ 
%		$\vph_3= (x_3 \rightarrow x_4) \lor (\neg x_3 \rightarrow \neg x_4)$.
%	\end{center}
%
%	\vspace{-0.7cm}
%
...
%	\vspace{-0.5cm}
%
%	For outcomes $M=(x_1,x_2,\neg x_3,\neg x_4)$, $M'=(\neg x_1,x_2,x_3,x_4)$,
%	and $M''=(\neg x_1,\neg x_2,x_3,\neg x_4)$,
%	we have $l_T(M)=l_T(M')=l_3$ and $l_T(M'')=l_4$, so $M \approx_T M' \succ_T M''$.
%}
	...
	\begin{enumerate}
	  ...
	  \item every non-leaf node $t$ labeled with $\varphi$ has either
	        two outgoing edges (Figure (a)), or one 
	    		outgoing edge pointing straight-down (Figure (b)),
					left (Figure (c)), or right (Figure (d)).
	\end{enumerate}
...
%	          }
%	        }
%					child {node [main node,inner sep=1.7pt] (9) {$\varphi_4$}
...
\frameT{From LP-Trees to P-Trees}{
	\begin{figure}[!ht]
		\centering
	  ...
	  \caption{P-trees extend LP-trees}
	\end{figure}
...
\vspace{-0.2cm}
	...
	%	$x_1x_2x_3x_4 \succ_r x_1\neg x_2x_3\neg x_4$,\\
	%	$\neg x_1x_2x_3x_4 \succ_r \neg x_1x_2\neg x_3x_4$,\\
	%	$\neg x_1x_2\neg x_3\neg x_4 \succ_r x_1\neg x_2\neg x_3x_4$.
	%\end{center}
...
\frameT{From ASO-Rules to P-Trees}{
	...
	\begin{thm}
		...
		polynomial in the size of $r$ such that for every two outcomes $M$ and $M'$
			\vspace{-0.2cm}
		\begin{center}
			$M \succeq_r M' \;\; \tit{iff} \;\; M \succeq_{T_r} M'$
		\end{center}
	\end{thm}
}
...
%\frameT{From P-trees to ASO-Theories}{
%	\begin{figure}
%	  \centering
%	  \subfigure[P-tree $T$]{
%	    \centering
...
%	    \end{tikzpicture}
%	  }
%		\subfigure[RASO-theory $\Phi_T$]{
%			\begin{tikzpicture}[->,>=stealth,node distance=0.5cm,main node/.style={rectangle,font=\small}]
%		    \node[main node] (1) {$x_1 \vee x_3 \overset{1}{\leftarrow}$};
%		    \node[main node] (2) [below of=1] {$x_1 \land \neg x_2 \overset{2}{\leftarrow} x_1 \vee x_3$};
%		    \node[main node] (3) [below of=2] {$x_2 \land x_4 \overset{2}{\leftarrow} \neg (x_1 \vee x_3)$};
%		    \node[main node] (4) [below of=3] {$(x_3 \rightarrow x_4) \lor (\neg x_3 \rightarrow \neg x_4) \overset{3}{\leftarrow} x_1 \vee x_3$};
%		    \node[main node] at (0,-2) {};
%		  \end{tikzpicture}
%		}
%	  \caption{P-trees}
%	\end{figure}
...
\frameT{From P-trees to ASO-Theories}{
	...
	\begin{thm}
		...
		polynomial in the size of $T$ such that for every two outcomes $M$ and $M'$
		\begin{center}
			$M \succeq_{\Phi_T} M' \;\; \tit{iff} \;\; M \succeq_{T} M'$
		\end{center}
	\end{thm}
}
...
\frameT{From Poss-theories to P-Trees}{
...
\frameT{Expressivity of P-Trees}{
	\begin{center}
		ASO-rules = Poss-theories $\subset$ \dual{LP-trees/\dual{$\cap$/P-trees}} $\subset$ ASO-theories
	\end{center}
}
...
\frameT{Decision Problems}{
	...
	\begin{block}{Optimality-testing ({\sc OptTest})}
		Given a P-tree $T$ and an outcome $M$,
		decide whether $M$ is an optimal outcome with respect to $T$.
	\end{block}
	  ...
	  \begin{tabular}[0.5\textwidth]{ | c | c | c | c | }
	    \hline
	     & {\sc DomTest}& {\sc OptTest} & {\sc OptProp} \\
			...
			LP-tree & P & P & P \\
			\hline
			\pbox{20cm}{ASO-rule/ \\ Poss-theory} & P & coNP-complete & $\deltap{2}$($P^{NP}$) \\
	    \hline
	    P-tree & P & coNP-complete\fn{The complement problem is reduced from the SAT problem.}
				& $\deltap{2}$($P^{NP}$)-complete\fn{The problem is reduced from the Maximum Satisfying 
																					Assignment (MSA) problem.}\\
			\hline
			ASO-theory & P & coNP-complete & $\sigmap{2}$($NP^{NP}$)-complete \\
	    \hline
	  \end{tabular}
